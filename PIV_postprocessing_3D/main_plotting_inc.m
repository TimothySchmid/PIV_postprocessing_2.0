...
%
% DESCRIPTION
%   loads cleaned .mat files for plotting. Topography, displacements and
...
%
%   - INPUT.temporal_smoothing --> integer; how many timesteps should be
%   used for temporal smoothing. 1 --> no smoothing
%
...
%           - 'Dw' z-displacement component
%           - 'D2' total displacement in plane
%           - 'D3' total displacement including Dz
%
%   STRAINS
%   ====================================================================  %
...
%       - 'default' (choses
%          existing suggested fixed values for incremental data)
...
% February 2023; Last revision: 20/02/2023 
...
% ======================================================================= %
  ...
  INPUT.datatype            = 'vc_inc';
  
  INPUT.temporal_smoothing  = 5;
  ...
  INPUT.set_clim            = [-0.02, 0.02];

  INPUT.plot_vectors        = 'no';
  INPUT.yvectors            = 8;
  INPUT.vec_color           = [0.75 0.25 0.25];

  INPUT.check_origin        = 'no';
  INPUT.xcorrection         = -86;
  INPUT.ycorrection         =  35;
...
for iRead = (10:10:100) % start_position : increment : end_position
    
cd(path_to_data)

% TEMPORAL SMOOTHING FOR DISPLACEMENT COMPONENTS
% ----------------------------------------------------------------------- %
[Du, Dv] = fct_temporal_smoothing(INPUT,iRead,n,nx,ny,files);
...
% clean boundaries
is_valid = fct_prepare_mask(logical(is_valid),5);
is_valid = single(is_valid);

is_valid(is_valid==0) = NaN;

Du = Du .* single(is_valid);
Dv = Dv .* single(is_valid);
Dw = Dw .* single(is_valid);
TOPO = TOPO .* single(is_valid);
...
switch INPUT.plot_choice
    case 'topography'
        % Get topography ready
        PLT = fct_prepare_topography(TOPO);
   
    case 'displacement'
        % Get displacements ready
        PLT = fct_prepare_displacement(INPUT,Du,Dv,Dw);
    case 'strain'
...
switch INPUT.plot_choice
    case 'topography'
        % Plot topo data
        fct_plot_topo(X,Y,PLT,ch,valid_x,valid_y)
    case 'displacement'
